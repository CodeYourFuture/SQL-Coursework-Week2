    ...
    pool
      ...
      .then((result) => {
        if (result.rows.length > 0) {
          ...
          pool
            .query(queryToCheckProdIdExists, [prod_id])
            .then((result) => {
              if (result.rows.length === 0) {
                res
                  .status(400)
                ...
                const queryToCheckSuppIdExists = "SELECT * FROM suppliers WHERE id=$1";
                pool.query(queryToCheckSuppIdExists, [supp_id])
                .then((result) => {
                  if(result.rows.length === 0) {
                    res.status(400).send(`The supplier id of ${supp_id} does not exists. Please make sure that both product id and supplier id are valid.`);
                  } else {
